...
struct MANGOS_DLL_DECL example_creatureAI : public ScriptedAI
{
    ...
    uint32 m_uiSpell_2_Timer;                               //Timer for spell 2 when in combat
    uint32 m_uiSpell_3_Timer;                               //Timer for spell 3 when in combat
    ...
    void UpdateAI(const uint32 uiDiff)
    {
        ...
        if (!m_creature->getVictim())
        {
            ...
            if (m_uiSay_Timer < uiDiff)
            {
                ...
                m_uiSay_Timer = 45000;                      //Say something again in 45 seconds
            }
            else
            ...
            if (m_uiRebuff_Timer < uiDiff)
            {
                ...
                m_uiRebuff_Timer = 900000;                  //Rebuff again in 15 minutes
            }
            else
        ...
        if (m_uiPhase > 1)
        {
            ...
            if (m_uiBeserk_Timer < uiDiff)
            {
                ...
                //Cast our beserk spell again in 12 seconds if we didn't kill everyone
